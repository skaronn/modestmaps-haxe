 ...
 * 	<li><strong> skipFailed : Bool</strong> - If <code>skipFailed</code> is <code>true</code> (the default), any failed loaders in the queue will be skipped. Otherwise, the LoaderMax will stop when it hits a failed loader and the LoaderMax's status will become <code>LoaderStatus.FAILED</code>.</li>
 * 	<li><strong> skipPaused : Bool</strong> - If <code>skipPaused</code> is <code>true</code> (the default), any paused loaders in the queue will be skipped. Otherwise, the LoaderMax will stop when it hits a paused loader and the LoaderMax's status will become <code>LoaderStatus.FAILED</code>.</li>
 * 	<li><strong> autoLoad : Bool</strong> - If <code>true</code>, the LoaderMax instance will automatically call <code>load()</code> whenever you insert()/append()/prepend() a new loader whose status is <code>LoaderStatus.READY</code>. This basically makes it easy to create a LoaderMax queue and dump stuff into it whenever you want it to load without having to check the LoaderMax's status and call <code>load()</code> manually if it's not already loading.</li>
 ...
 * 	<li><strong> onComplete : Dynamic</strong> - A handler function for <code>LoaderEvent.COMPLETE</code> events which are dispatched when the loader has finished loading. Make sure your onComplete function accepts a single parameter of type <code>LoaderEvent</code> (<code>com.greensock.events.LoaderEvent</code>).</li>
 ...
 * 	<li><strong> onHTTPStatus : Dynamic</strong> - A handler function for <code>LoaderEvent.HTTP_STATUS</code> events. Make sure your onHTTPStatus function accepts a single parameter of type <code>LoaderEvent</code> (<code>com.greensock.events.LoaderEvent</code>).</li>
...
/** @private **/
public static inline var version:Float = 1.939;
 ...
 */
public static function parse(data:Dynamic, vars:Map<String, Int>=null, childrenVars:Map<String, Int>=null):Dynamic {
	if (data is Array) {
...
private function _removeLoader(loader:LoaderCore, rootLoaderAppend:Bool):Void {
	...
	untyped __delete__(_activeLoaders[loader];
	loader.cancel();
	if (_status == LoaderStatus.LOADING) {
		_loadNext(null);
	}
...
override private function _dump(scrubLevel:Int=0, newStatus:Int=0, suppressEvents:Bool=false):Void {
	...
	if (this.vars.requireWithRoot is DisplayObject) {
		untyped __delete__(_rootLookup[this.vars.requireWithRoot];
	}
	_activeLoaders = null;
	}
	if (scrubLevel <= 1) {
...
private function _auditSize(event:Event=null):Void {
	...
	var loader:LoaderCore, found:Bool;
	for (var i:Int = 0; i < l; i++) {
	loader = _loaders[i];
...
private function _loadNext(event:Event=null):Void {
	...
	untyped __delete__(_activeLoaders[event.target];
	_removeLoaderListeners(LoaderCore(event.target), false);
	}
	
	if (_status == LoaderStatus.LOADING) {
	
	var audit:Bool = ("auditSize" in this.vars) ? Boolean(this.vars.auditSize) : LoaderMax.defaultAuditSize;
...
/** Float of child loaders currently contained in the LoaderMax instance (does not include deeply nested loaders - only children). To get the quantity of all children including nested ones, use <code>getChildren(true, true).length</code> @see #getChildren() **/
public var numChildren(getNumChildren, null):UInt;
 	private function getNumChildren():UInt {
	return _loaders.length;
}

/** An array containing the content of each loader inside the LoaderMax **/
override public var content(getContent, null):Dynamic;
...
/** @inheritDoc **/
override public var status(getStatus, null):Int;
...
/** @inheritDoc **/
override public var auditedSize(getAuditedSize, null):Bool;
 	private function getAuditedSize():Bool {
	var maxStatus:Int = (this.skipPaused) ? LoaderStatus.COMPLETED : LoaderStatus.PAUSED;
	var i:Int = _loaders.length;
	while (--i > -1) {
 ...
 * @see #progress
 **/
public var rawProgress(getRawProgress, null):Float;
 	private function getRawProgress():Float {
	var loaded:Float = 0;
	var total:UInt = 0;
	var status:Int;
	var i:Int = _loaders.length;
