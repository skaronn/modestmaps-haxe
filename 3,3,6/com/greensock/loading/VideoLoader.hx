 ...
 * 	<li><strong> bufferTime : Float</strong> - The amount of time (in seconds) that should be buffered before the video can begin playing (set <code>autoPlay</code> to <code>false</code> to pause the video initially).</li>
 * 	<li><strong> autoPlay : Bool</strong> - By default, the video will begin playing as soon as it has been adequately buffered, but to prevent it from playing initially, set <code>autoPlay</code> to <code>false</code>.</li>
 * 	<li><strong> smoothing : Bool</strong> - When <code>smoothing</code> is <code>true</code> (the default), smoothing will be enabled for the video which typically leads to better scaling results.</li>
 ...
 * 	<li><strong> width : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>width</code> property (applied before rotation, scaleX, and scaleY).</li>
 * 	<li><strong> height : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>height</code> property (applied before rotation, scaleX, and scaleY).</li>
 * 	<li><strong> centerRegistration : Bool </strong> - if <code>true</code>, the registration point will be placed in the center of the <code>ContentDisplay</code> which can be useful if, for example, you want to animate its scale and have it grow/shrink from its center.</li>
 ...
 * 		</ul></li>
 * 	<li><strong> crop : Bool</strong> - When a <code>width</code> and <code>height</code> are defined, setting <code>crop</code> to <code>true</code> will cause the video to be cropped within that area (by applying a <code>scrollRect</code> for maximum performance). This is typically useful when the <code>scaleMode</code> is <code>"proportionalOutside"</code> or <code>"none"</code> so that any parts of the video that exceed the dimensions defined by <code>width</code> and <code>height</code> are visually chopped off. Use the <code>hAlign</code> and <code>vAlign</code> special properties to control the vertical and horizontal alignment within the cropped area.</li>
 * 	<li><strong> x : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>x</code> property (for positioning on the stage).</li>
 * 	<li><strong> y : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>y</code> property (for positioning on the stage).</li>
 * 	<li><strong> scaleX : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>scaleX</code> property.</li>
 * 	<li><strong> scaleY : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>scaleY</code> property.</li>
 * 	<li><strong> rotation : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>rotation</code> property.</li>
 * 	<li><strong> alpha : Float</strong> - Sets the <code>ContentDisplay</code>'s <code>alpha</code> property.</li>
 * 	<li><strong> visible : Bool</strong> - Sets the <code>ContentDisplay</code>'s <code>visible</code> property.</li>
 ...
 * 	<li><strong> noCache : Bool</strong> - If <code>noCache</code> is <code>true</code>, a "gsCacheBusterID" parameter will be appended to the url with a random set of numbers to prevent caching (don't worry, this info is ignored when you <code>getLoader()</code> or <code>getContent()</code> by url and when you're running locally)</li>
...
private function _statusHandler(event:NetStatusEvent):Void {
	...
	var videoRemaining:Float = this.duration - this.videoTime;
	var prevBufferMode:Bool = _bufferMode;
	_bufferMode = false; //make sure bufferMode is false so that when we check progress, it gives us the data we need.
	_cacheIsDirty = true;
	var prog:Float = this.progress;
	...
	var loadRemaining:Float = (1 / prog) * this.loadTime;
	var revisedBufferTime:Float = videoRemaining * (1 - (videoRemaining / loadRemaining)) * 0.9; //90% of the estimated time because typically you'd want the video to start playing again sooner and the 10% might be made up while it's playing anyway.
...
override public var content(getContent, setContent):Dynamic;
...
public var rawContent(getRawContent, setRawContent):Video;
 	private function getRawContent():Video {
	return _video;
}
...
public var videoPaused(getVideoPaused, setVideoPaused):Bool;
 	private function getVideoPaused():Bool {
	return _videoPaused;
}
private function setVideoPaused(value:Bool):Void {
	var changed:Bool = Boolean(value != _videoPaused);
	_videoPaused = value;
	if (_videoPaused) {
...
public var bufferProgress(getBufferProgress, setBufferProgress):Float;
 	private function getBufferProgress():Float {
...
public var playProgress(getPlayProgress, setPlayProgress):Float;
 	private function getPlayProgress():Float {
...
private function setPlayProgress(value:Float):Void {
	if (_duration != 0) {
...
private function setVolume(value:Float):Void {
...
public var soundTransform(getSoundTransform, setSoundTransform):SoundTransform;
 	private function getSoundTransform():SoundTransform {
	return _sound;
}
private function setSoundTransform(value:SoundTransform):Void {
...
public var videoTime(getVideoTime, setVideoTime):Float;
 	private function getVideoTime():Float {
...
private function setVideoTime(value:Float):Void {
 ...
 **/
public var bufferMode(getBufferMode, setBufferMode):Bool;
 	private function getBufferMode():Bool {
	return _bufferMode;
}
private function setBufferMode(value:Bool):Void {
	_bufferMode = value;
...
public var autoDetachNetStream(getAutoDetachNetStream, setAutoDetachNetStream):Bool;
 	private function getAutoDetachNetStream():Bool {
	return _autoDetachNetStream;
}
private function setAutoDetachNetStream(value:Bool):Void {
