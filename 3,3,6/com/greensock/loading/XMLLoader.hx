 ...
 * 	<li><strong> autoDispose : Bool</strong> - When <code>autoDispose</code> is <code>true</code>, the loader (and its children) will be disposed immediately after it completes (it calls the <code>dispose()</code> method internally after dispatching its <code>COMPLETE</code> event). This will remove any listeners that were defined in the vars object (like onComplete, onProgress, onError, onInit). Once a loader is disposed, it can no longer be found with <code>LoaderMax.getLoader()</code> or <code>LoaderMax.getContent()</code> - it is essentially destroyed but its content is not unloaded (you must call <code>unload()</code> or <code>dispose(true)</code> to unload its content). The default <code>autoDispose</code> value is <code>false</code>.</li>
...
private var _loadingQueue:LoaderMax;
...
/** @private **/
private static function _parseVars(xml:XML):Map<String, Int> {
	var v:Map<String, Int> = {rawXML:xml};
...
public static function parseLoaders(xml:XML, all:LoaderMax, toLoad:LoaderMax=null):Void {
	...
	if (nodeName in _types) {
		...
		var parsedVars:Map<String, Int> = _parseVars(xml);
		if (Type.typeof(all.vars.childrenVars) == "object") {
...
override private function _receiveDataHandler(event:Event):Void {
	...
	_loadingQueue = new LoaderMax({name:this.name + "_Queue", maxConnections:(uint(this.vars.maxConnections) || 2), skipFailed:Bool(this.vars.skipFailed != false), skipPaused:Bool(this.vars.skipPaused != false)});
...
override public var progress(getProgress, null):Float;
