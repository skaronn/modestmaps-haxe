...
public function updateTo(vars:Object, resetDuration:Bool=false):Dynamic {
	...
	if (resetDuration) {
		...
		var inv:Float = 1 / (1 - curRatio),
			pt:PropTween = _firstPT, endValue:Float;
		while (pt) {
			endValue = pt.s + pt.c; 
			pt.c *= inv;
			pt.s = endValue - pt.c;
...
override public function render(time:Float, suppressEvents:Bool=false, force:Bool=false):Void {
	...
	prevTotalTime:Float = _totalTime, 
	prevCycle:Float = _cycle, 
	isComplete:Bool, callback:String, pt:PropTween, rawPrevTime:Float;
	if (time >= totalDur) {
	_totalTime = totalDur;
	_cycle = _repeat;
	if (_yoyo && (_cycle & 1) != 0) {
	...
	if (_easeType) {
		var r:Float = _time / _duration, type:Int = _easeType, pow:Int = _easePower;
...
public static function killAll(complete:Bool=false, tweens:Bool=true, delayedCalls:Bool=true, timelines:Bool=true):Void {
	...
	l:Int = a.length,
	isDC:Bool,
	allTrue:Bool = (tweens && delayedCalls && timelines),
