...
class TweenLite extends Animation {
	...
	public function new(target:ObjectMap<Dynamic, Dynamic>, duration:Float, vars:ObjectMap<Dynamic, Dynamic>) {
		...
		if (this.vars.get("immediateRender") || (duration == 0 && _delay == 0 && this.vars.get("immediateRender") != false)) {
	...
	private function _init():Void {
		...
		if (vars.get("startAt")) {
			...
			vars.get("startAt").set("overwrite", 0);
			//vars.startAt.immediateRender = true;
			vars.get("startAt").set("immediateRender", true);
			//_startAt = new TweenLite(target, 0, vars.startAt);
			_startAt = new TweenLite(target, 0, vars.get("startAt"));
			if (immediate) {
				if (_time > 0) {
		...
		else if (vars.get("runBackwards") && _duration != 0) {
		...
		//} else if (typeof(vars.ease) === "function") {
			...
			_ease = new Ease(vars.get("ease"), vars.get("easeParams"));
		} else {
			_ease = defaultEase;
		}
		...
		if (_overwrittenProps != null) if (_firstPT == null) if (!Std.is(Type.typeof(target), Dynamic)) { //if all tweening properties have been overwritten, kill the tween. If the target is a function, it's most likely a delayedCall so let it live.
			_enabled(false, false);
		}
		//if (vars.runBackwards) {
		if (vars.get("runBackwards")) {
			pt = _firstPT;
			while (pt != null) {
				pt.s += pt.c;
				pt.c = -pt.c;
	...
	override public function render(time:Float, suppressEvents:Bool=false, force:Bool=false):Void {
		...
		if (prevTime == 0) {
			...
			if (vars.get("onStart")) if (_time != 0 || _duration == 0) if (!suppressEvents) {
				//vars.onStart.apply(null, vars.onStartParams);
				Reflect.callMethod(this, vars, [ "onStartParams" ]);
			}
		}
		...
		if (_onUpdate != null) {
			...
			if (!suppressEvents) if (_time != prevTime || isComplete) {
				_onUpdate.apply(null, vars.get("onUpdateParams"));
			}
		}
	...
	private static function _prepVars(vars:Dynamic, immediateRender:Bool=false):Dynamic {
		...
		if (immediateRender && Reflect.field(vars, "immediateRender") != null) {
	...
	private static function _applyOverwrite(target:ObjectMap<Dynamic, Dynamic>, tween:TweenLite, props:ObjectMap<Dynamic, Dynamic>, mode:Int, siblings:Array<Dynamic>):Bool {
		...
		while (--i > -1) {
			...
			if (mode != 2 || (curTween._firstPT != null && curTween._initted)) {
	...
	private static function _checkOverlap(tween:Animation, reference:Float, zeroDur:Bool):Float {
		...
		ts:Float = tl._timeScale, 
		t:Float = tween._startTime,
		min:Float = 0.0000000001;
