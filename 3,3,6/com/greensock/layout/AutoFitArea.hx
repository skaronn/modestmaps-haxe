 ...
 */
public function new(parent:DisplayObjectContainer, x:Float=0, y:Float=0, width:Float=100, height:Float=100, previewColor:UInt=0xFF0000) {
	super();
	super.x = x;
	super.y = y;
	if (parent == null) {
 ...
 * 			<li><b>crop : Bool</b> - If true, a mask will be created and added to the display list so that the target will be cropped wherever it exceeds the bounds of the AutoFitArea.</li>
 * 			<li><b>roundPosition : Bool</b> - To force the target's x/y position to snap to whole pixel values, set <code>roundPosition</code> to <code>true</code> (it is <code>false</code> by default).</li>
 ...
 * 			<li><b>minWidth : Float</b> - Minimum width to which the target is allowed to scale</li>
 * 			<li><b>maxWidth : Float</b> - Maximum width to which the target is allowed to scale</li>
 * 			<li><b>minHeight : Float</b> - Minimum height to which the target is allowed to scale</li>
 * 			<li><b>maxHeight : Float</b> - Maximum height to which the target is allowed to scale</li>
 * 			<li><b>calculateVisible : Bool</b> - If true, only the visible portions of the target will be taken into account when determining its position and scale which can be useful for objects that have masks applied (otherwise, Flash reports their width/height and getBounds() values including the masked portions). Setting <code>calculateVisible</code> to <code>true</code> degrades performance, so only use it when absolutely necessary.</li>
...
public function update(event:Event=null):Void {
	...
	var item:AutoFitItem = _rootItem;
	var w:Float, h:Float, tx:Float, ty:Float, target:DisplayObject, innerBounds:Rectangle, outerBounds:Rectangle, tRatio:Float, scaleMode:String, ratio:Float, angle:Float, sin:Float, cos:Float, m:Matrix, wScale:Float, hScale:Float, mPrev:Matrix;
	while (item) {
	target = item.target;
...
/** @inheritDoc **/
override public var width(null, setWidth):Float;
...
/** @inheritDoc **/
override public var height(null, setHeight):Float;
...
/** @inheritDoc **/
override public var scaleX(null, setScaleX):Float;
 	private function setScaleX(value:Float):Void {
	super.scaleX = value;
	update();
}

/** @inheritDoc **/
override public var scaleY(null, setScaleY):Float;
 	private function setScaleY(value:Float):Void {
	super.scaleY = value;
	update();
}
...
public var previewColor(getPreviewColor, setPreviewColor):UInt;
 	public var preview(getPreview, setPreview):Bool;
 	private function getPreviewColor():UInt {
	return _previewColor;
}
private function setPreviewColor(value:UInt):Void {
	_redraw(value);
}
...
public var preview(getPreview, setPreview):Bool;
 	private function getPreview():Bool {
	return _preview;
}
private function setPreview(value:Bool):Void {
	_preview = value;
