...
class Animation {
	...
	//add listener
	Animation.ticker.addEventListener("tick", myDynamic);
	...
	Animation.ticker.removeEventListener("tick", myDynamic);
	</listing>
	 * 
	...
	Animation.ticker.addEventListener("tick", myDynamic, this, true, 1);
	...
	Animation.ticker.addEventListener("tick", myDynamic, false, 0, true);
	...
	/** @private Records the parent timeline's <code>rawTime</code> when the animation is paused (so that we can place it at the appropriate time when it is unpaused). 0 when the animation isn't paused. **/
	...
	public function seek(time:Dynamic, suppressEvents:Bool=true):Dynamic {
		return totalTime(cast(time, Float), suppressEvents);
	}
	...
	public function _kill(vars:Map<String, Int>=null, target:Map<String, Int>=null):Bool {
	 ...
	 * @return self (makes chaining easier)
	 **/
	public function kill(vars:Map<String, Int>=null, target:Map<String, Int>=null):Dynamic {
		_kill(vars, target);
		return this;
	}
	...
	var myAnimation = new TweenLite(mc, 1, {x:100, onComplete:myDynamic, onCompleteParams:["param1","param2"]});
	myAnimation.eventCallback("onComplete", myDynamic, ["param1","param2"]);
	</listing>
	 ...
	 * callback references or even untyped __delete__(them on-the-fly (use <code>null</code> to untyped __delete__(the 
	 * event callback).</p>
	 * 
	 * <listing version="3.0">
	...
	myAnimation.eventCallback("onComplete", myDynamic); //sets the onComplete
	</listing>
	 * 
	...
	public function startTime(value:Float=0):Dynamic {
		...
		if (value != _startTime) {
			_startTime = value;
			if (timeline != null) if (timeline._sortChildren) {
	...
	public function paused(value:Bool=false):Dynamic {
		...
		if (value != _paused) if (_timeline != null) {
			...
			elapsed:Float = raw - _pauseTime;
			if (!value && _timeline.smoothChildTiming) {
				_startTime += elapsed;
				_uncache(false);
			}
			_pauseTime = value ? raw : 0;
			_paused = value;
