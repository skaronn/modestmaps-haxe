 ...
 * 
 * 	<li><strong>curviness</strong> : Float (default:1) (only applies to <code>type:"thru"</code>) - allows you to adjust the 
 ...
 * 
 * 	<li><strong>autoRotate</strong> : Bool or Array (default:false) - to automatically rotate the target according
...
private static const _RAD2DEG:Float = 180 / Math.PI; 
 ...
 */
private static function _calculateControlPoints(a:Array, curviness:Float=1, quad:Bool=false, basic:Bool=false, correlate:Bool=false):Void {
	var l:Int = a.length - 1,
	ii:Int = 0,
	cp1:Float = a[0].a,
	i:Int, p1:Float, p2:Float, p3:Float, seg:Segment, m1:Float, m2:Float, mm:Float, cp2:Float, qb:Array, r1:Float, r2:Float, tl:Float;
	for (i = 0; i < l; i++) {
	seg = a[ii];
	p1 = seg.a;
	p2 = seg.d;
...
public static function cubicToQuadratic(a:Float, b:Float, c:Float, d:Float):Array<Dynamic> {
	...
	mbc:Float = (b + c) / 2, 
	mcd:Float = (c + d) / 2, 
	mabc:Float = (mab + mbc) / 2,
	mbcd:Float = (mbc + mcd) / 2,
	m8:Float = (mbcd - mabc) / 8;
	q1.b = mab + (a - mab) / 4;	
	q2.b = mabc + m8;
...
private static function _parseLengthData(obj:Map<String, Int>, precision:UInt=6):Map<String, Int> {
	...
	p:String, i:Int, l:Int, index:Float;
	for (p in obj) {
...
private static function _addCubicLengths(a:Array, steps:Array, precision:UInt=6):Void {
	...
	j:Int = a.length,
	d:Float, d1:Float, s:Float, da:Float, ca:Float, ba:Float, p:Float, i:Int, inv:Float, bez:Segment, index:Int;
	while (--j > -1) {
	bez = a[j];
	s = bez.a;
	da = bez.d - s;
...
override public function setRatio(v:Float):Void {
	...
	var ar:Array = this._autoRotate,
		b2:Segment, x1:Float, y1:Float, x2:Float, y2:Float, add:Float, conv:Float;
	i = ar.length;
	while (--i > -1) {
		p = ar[i][2];
		add = ar[i][3] || 0;
...
public function Segment(a:Float, b:Float, c:Float, d:Float) {
this.a = a;
this.b = b;
this.c = c;
this.d = d;
