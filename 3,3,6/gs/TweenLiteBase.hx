...
class TweenLite {
	...
	private static var _all:ObjectMap<Dynamic, Dynamic> = new ObjectMap<Dynamic, Dynamic>(); //Holds references to all our tween targets.
	...
	public function new(target:Dynamic, duration:Float, vars:Dynamic) {
		...
		{ 
			...
			_all.set(target, new ObjectMap<Dynamic, Dynamic>());
		}
		_all.set(target, this);
		this.vars = vars;
		this.duration = duration;
		var reset : Float = (vars.delay != null ? cast(vars.delay, Float) :  0);
		this.delay = reset;
	...
	public function initTweenVals():Void
	{
		...
		if (Std.is(this.target, Array))
		{
			...
			{
				if (this.target[i] != endArray[i] && this.target[i] != null)
				{
	...
	{		
		if (tg != null && _all.get(tg) != null)
		{
			if (complete)
			{
				var o:Array<Dynamic> = _all.get(tg);
				for (tw in o)
				{
					//o.get(tw).complete(false);
				}
			}
			untyped __delete__(_all, tg);
		}
	}
	...
	public function set_endTarget($t:Dynamic) : Void
	{
		...
		{//Otherwise subtweens (like for color or volume) that have a duration of 0 will stick around for 1 frame and get rendered in the executeAll() loop which means they'll render incorrectly
			...
			untyped __delete__(_all, _all[_endTarget][this]);
			_endTarget = $t;
		if (_all[$t] == undefined) {
	...
	public function set_volumeProxy($n:Float):Void { //Used as a proxy of sorts to control the volume of the target MovieClip.
		_sound.volume = $n;
		this.target.soundTransform = _sound;
	}
	
	public var volumeProxy(getVolumeProxy, null):Float;
		private function getVolumeProxy():Float {
		return _sound.volume;
	}
	
	public function set_colorProxy($n:Float):Void { 
		var r:Float = 1 - $n;
	...
	public var colorProxy(getColorProxy, null):Float;
		private function getColorProxy():Float {
		return 0;
	}
