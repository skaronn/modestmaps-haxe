...
class Main extends Sprite 
{
	...
	*/
	...
	{		
		...
		var locations : Array<Location> = [ new Location(37.7749300, -122.4194200), new Location(37.8043700, -122.2708000), new Location(37.8715900, -122.2727500) ];
		var extent : MapExtent = MapExtent.fromLocations(locations);		
		
		// create map		
		map = new TweenMap(stage.stageWidth - 2 * PADDING, stage.stageHeight - 2 * PADDING, 
				true, new GoogleMapProvider(), 
				[ { extent; } ]);
		...
		var spotlight:SpotlightOverlay = new SpotlightOverlay(map);
		var arcExtent:MapExtent = spotlight.addSpot(locations);
